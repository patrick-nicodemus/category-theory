...
Section Quiver.
  ...
  Fixpoint tlist'_quiver_equiv (k i j : G) 
    (f : @tlist' _ edges k i) (g : @tlist' _ edges k j) ( p : i = j )
    : Type :=
    match f in (tlist' _ i_t) return (tlist' _ j) -> i_t = j -> Type with
    | tnil => fun g' q =>
                tnil = Logic.transport_r (fun z => @tlist' _ edges k z) q g'
    | @tcons _ _ _ i0 i1 fhead ftail =>
        fun g' : tlist' _ j  => 
          match g' in tlist' _ j return
                @tlist' _ edges k i1 -> i0 = j -> Type with
          | tnil => fun _ _ => False
  ...
  Inductive tlist'_quiver_inductive (i : G) :
    forall (j : G),
      (@tlist' _ edges i j) -> (@tlist' _ edges i j) -> Type :=
  | tlist_both_empty : tlist'_quiver_inductive i tnil tnil
  | tlist_both_cons (k : G) :
    forall (j : G) (ftail gtail : @tlist' _ edges i j)
           (fhead ghead : edges k j),
           (tlist'_quiver_inductive j ftail gtail) ->
           fhead ≈ ghead ->
           tlist'_quiver_inductive k (tcons _ fhead ftail)
                                   (tcons _ ghead gtail).

  Proposition fixpoint_implies_inductive
    (i j : G) (f g : tlist edges i j)
    : tlist'_quiver_equiv _ _ _ f g eq_refl
      -> (tlist'_quiver_inductive _ _ f g).
  Proof.
    revert g; induction f.
    - intros g H. simpl in H; unfold Logic.transport_r in H;
  ...
  Lemma tlist'_equiv_lengths (i j k : G) (f : tlist' k i)
    (g : tlist' k j) (p : i = j)
    ...
    - intros j' g p; destruct g; [ now apply False_rect | simpl ].
      intros [j_eq_j0 X]; apply eq_S; unshelve eapply IHf;
        [ exact j_eq_j0 | assumption ].
  Qed.
  ...
  Proof.
    ...
    - destruct j1_eq_k1, p, q, j_eq_j0.
      ...
      simpl Logic.transport in *.
      now apply (@Equivalence_Transitive _ _ _ _ ghead).
    - unshelve eapply IHftail; [ exact gtail | assumption | assumption ].
  Qed.

  Proposition tlist'_quiver_equiv_symmetric (k i j: G) (p : i = j)
    (f : tlist edges i k) (g: tlist edges j k) 
    : tlist'_quiver_equiv k i j f g p ->
      tlist'_quiver_equiv k j i g f (eq_sym p).
  Proof.
    revert j p g; induction f as [| i0 i1 fhead ftail];
...
#[export]
  Instance tlist_quiver_equiv_is_equiv (i k : G)
    : Equivalence (tlist_quiver_equiv i k).
  Proof.
    unshelve eapply Build_Equivalence.
    + intro; apply tlist'_quiver_equiv_reflexive.
    + intros f g H;
        unfold tlist_quiver_equiv;
        change (@eq_refl _ i) with (eq_sym (@eq_refl _ i));
    ...
    + intros f g h equiv1 equiv2. unfold tlist_quiver_equiv.
      ...
      unshelve eapply tlist'_quiver_equiv_transitive;
        [ exact g | |]; assumption.
  Defined.
End Quiver.
...
Proposition transport_id (C : Category) (c c' : C) (p : c = c')
  : Logic.transport (hom c) p (@id C c) =
    Logic.transport_r (fun z => hom z c') p (@id C c').
Proof.
  now destruct p.
Defined.
...
Proof.
  ...
  apply (proper_transport_dom Q' edges _ (F c) (G c) (G c')
           (F_eq_G_on_obj c)) in
    FG_coherent_transport.
  rewrite transport_trans, eq_trans_sym_inv_l
    in FG_coherent_transport;
    simpl in FG_coherent_transport.
  apply (proper_transport_cod Q' edges _ (G c) (G c') (F c')
           (eq_sym (F_eq_G_on_obj c'))) in
    FG_coherent_transport.
...
Lemma transport_quiver_dom
  (C D: Type)
  (ArrC : forall c c' : C, Type)
  (ArrD : forall d d' : D, Type)
  (Fobj : C -> D)
  (Fmap : forall c c' : C, (ArrC c c') -> (ArrD (Fobj c) (Fobj c')))
  (c c' d: C)
  (p : c = c')
  (f : ArrC c d)
  : Fmap _ _ (Logic.transport (fun z => ArrC z d) p f) =
      Logic.transport (fun z => ArrD z (Fobj d))
        (f_equal (Fobj) p) (Fmap _ _ f).
Proof.
...
Lemma transport_quiver_cod
  ...
  : Fmap _ _ (Logic.transport (fun z => ArrC c z) p f) =
      Logic.transport (fun z => ArrD (Fobj c) z)
        (f_equal (Fobj) p) (Fmap _ _ f).
Proof.
  destruct p. reflexivity.
Defined.
...
Proof.
  unshelve refine
    (Build_QuiverHomomorphism Q1 Q3
       (Basics.compose (@fnodes _ _ G) (@fnodes _ _ F)) _ _).
  unshelve eapply Build_SetoidMorphism.
  + exact (fun f => (fedgemap (fedgemap f))).
  + cat_simpl.
Defined.

#[export] Instance QuiverHomomorphismEquivalence_IsEquivalence
  (Q Q': Quiver)
  : @Equivalence (QuiverHomomorphism Q Q')
      (QuiverHomomorphismEquivalence Q Q') :=
  {|
...
Proof.
  ...
  + exact (fun Q Q' =>
             {| equiv := QuiverHomomorphismEquivalence Q Q';
               setoid_equiv :=
                 QuiverHomomorphismEquivalence_IsEquivalence Q Q'
             |}).
  + abstract(
    ...
    apply (proper_transport_cod _ edges _ (G1 (F1 x)) _ _
             (G_equiv_on_obj (F2 y)));
    ...
    reflexivity).
  + abstract(intros x y f; now exists (fun x =>  eq_refl)). 
  + abstract(intros x y f; now exists (fun x =>  eq_refl)). 
  + abstract(intros w x y z f g h; now exists (fun x => eq_refl)). 
Defined.
End QuiverCategory.
...
Section Underlying.
  ...
  Proof.
    ...
    + abstract(intros x y f g f_eq_g;
      destruct f_eq_g as [fg_eq_on_obj arrow_coherence];
      exists fg_eq_on_obj; simpl; intros c c' k;
      apply arrow_coherence).
    + abstract(intro C; exists (fun x => eq_refl); now trivial).
    + abstract(intros x y z f g; exists (fun x => eq_refl); now trivial).
  Defined.
...
Section Free.
  ...
  Program Definition FreeOnQuiver : Category@{o h p} :=
    {|
      ...
      hom     := tlist edges;
      homset  := fun i j =>
                   {| equiv := tlist_quiver_equiv _ _ _  ;
